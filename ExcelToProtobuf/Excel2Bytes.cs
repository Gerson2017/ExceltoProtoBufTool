...
namespace ExcelToProtobuf
{
    ...
    public class Excel2Bytes
    {
        ...
        public static void Data(string dllDir, string excelDir)
        {
            ...
            {
                Console.WriteLine($"{dllFilePath}:dll文件路径不存在！！！ 请检查是否配置CSC环境");
                return;
            }
            assembly = Assembly.LoadFile(dllFilePath);
            if (Directory.Exists(Common.ProtoDataPath)) // 导数据前删除里面的所有文件
            {
                Directory.Delete(Common.ProtoDataPath, true);
            }
            Directory.CreateDirectory(Common.ProtoDataPath);

            if (Directory.Exists(Common.LawsDataPath))
            {
                Directory.Delete(Common.LawsDataPath, true);
            }
            Directory.CreateDirectory(Common.LawsDataPath);
        ...
        private static void Serialize(object obj, string sheetName)
        {
            ...
            using (FileStream fs = new FileStream(Path.Combine(Common.ProtoDataPath, fileName), FileMode.Create, FileAccess.ReadWrite, FileShare.ReadWrite))
            {
        ...
        private static void WriteLawsData(object obj, string sheetName)
        {
            ...
            using (FileStream fs = new FileStream(Path.Combine(Common.LawsDataPath, fileName), FileMode.Create, FileAccess.ReadWrite, FileShare.ReadWrite))
            {
        ...
        static void CopyFileToTarget()
        {
            string exePath = Directory.GetCurrentDirectory();
            string parentPath = Directory.GetParent(exePath).FullName;
            string topPath = Directory.GetParent(parentPath).FullName;
            string configpath = Path.Combine(topPath, "Config/PathConfig.txt");

            string[] allconfigtext = File.ReadAllLines(configpath);

      
            if (allconfigtext.Length > 1)
            {
                //配置输出文件夹
                string ProtoDataPath = @allconfigtext[0];
               string CsharpPtah = @allconfigtext[1];

                ProtoDataPath = Path.GetFullPath(ProtoDataPath);
                CsharpPtah = Path.GetFullPath(CsharpPtah);


                Copy(Common.ProtoDataPath,ProtoDataPath);
                Copy(Common.CsharpPtah,CsharpPtah);
            }
        }
        ...
        public static void Copy(string srcpath, string targetpath)
        {
            ...
            {
                Console.WriteLine("无法复制：" + srcpath + " 到" + targetpath + " error:" + e);
            }
        }
